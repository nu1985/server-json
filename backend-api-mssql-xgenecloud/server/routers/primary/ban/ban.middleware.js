...
class BanMiddleware extends BaseMiddleware {

  constructor(app) {
    super({
      app
    });
    this.permissions = require('./ban.policy').permissions;
    // this.$acl = app.acl;
    // this.$acl.addPolicy(require('./ban.policy').acl);
  }


  async default (req, res, next) {

    console.log('Ban middleware;')
