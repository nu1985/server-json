...
class ProvinceMiddleware extends BaseMiddleware {

  constructor(app) {
    super({
      app
    });
    this.permissions = require('./province.policy').permissions;
    // this.$acl = app.acl;
    // this.$acl.addPolicy(require('./province.policy').acl);
  }


  async default (req, res, next) {

    console.log('Province middleware;')
