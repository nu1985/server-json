...
class RubMiddleware extends BaseMiddleware {

  constructor(app) {
    super({
      app
    });
    this.permissions = require('./rub.policy').permissions;
    // this.$acl = app.acl;
    // this.$acl.addPolicy(require('./rub.policy').acl);
  }


  async default (req, res, next) {

    console.log('Rub middleware;')
