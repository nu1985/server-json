...
/* eslint-disable no-unused-vars  if (!user || !user.email)*/
      try {
  ...
  async signup(req, res, next) {
    ...
    let user = await this.users.findOne({where: `(user,eq,${req.body.user})`});

    if (user && user.username) {
      throw new Error({msg: `user '${req.body.user}'ผู้ใช้คนนี้ลงทะเบียนแล้ว`})
    }
    ...
    user = await this.users.findOne({where: `(user,eq,${req.body.user})`});
  ...
  async passwordChange(req, res) {
    ...
    if (req.isAuthenticated()) {
      ...
      const user = await this.users.findOne({where: `(email,eq,${req.user.email})`});
      const hashedPassword = await (currentPassword);
      if (hashedPassword !== user.password) {
        throw new Error('Current password is wrong');
      }
  ...
  async me(req, res) {
    return {msg: 'Not implemeneted me'};
  }

  async oauth(req, res) {
    return {msg: 'Not implemeneted oauth'};
  }

  async oauthCallback(req, res) {
    return {msg: 'Not implemeneted oauth-callback'};
  }
