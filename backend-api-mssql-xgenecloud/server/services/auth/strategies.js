...
passport.serializeUser(function ({id,firstname,provider_ids,email,provider_data, username,password, email_verified, roles, provider}, done) {
  done(null, {
    id,
    firstname,
    provider_ids,
...
class Strategy {
  ...
  local({app}) {
    passport.use(new PassportLocalStrategy({usernameField: 'username'},

      async function (username, password, done) {

        try {

          let user = await app.$dbs.primary.users.findOne({where: `(username,eq,${username})`})

          if (!user || !user.username) {
            return done({msg: `username ${username} is not registered!`});
          }
